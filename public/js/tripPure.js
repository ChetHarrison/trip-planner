         ...
         <div class="d-flex flex-wrap gap-2 mb-3">
           ${list.map(p => `
             <div style="flex: 1 1 calc(20% - 10px); min-width: 200px;">
               ...
               ${p.place_id && apiKey ? `<iframe width="100%" height="200" frameborder="0" style="margin-top:5px" src="https://www.google.com/maps/embed/v1/place?key=${apiKey}&q=place_id:${p.place_id}" allowfullscreen></iframe>` : ''}
             </div>`).join('')}
         </div>`
      : '';
...
export const calculateDate = (startDate, index) =>
  moment(startDate, 'YYYY-MM-DD').add(index, 'days').format('dddd, MMMM Do YYYY');
